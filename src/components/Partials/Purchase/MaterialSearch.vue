<template>
        ...
        <div class=" position-relative">
            <input type="text" class="form-control border-none px-0" @focusout="hideItemSearch" id="customer-select" v-model="itemsearch"
                ref="firstInput" :class="{ 'border-danger': error, 'search-input-p-w': selectedItem }"
                :placeholder="selectedItem == null ? '' : (selectedItem.name)"
                @focusin="showItemSearch" @keydown.enter.prevent="handleEnter" @keydown.up.prevent="goUp"
                @keydown.down.prevent="goDown" data-type="ITEMSEARCH" :data-id="index" >
...
export default {
    ...
    mounted() {
        if(this.current_material_id!=undefined) {
                let items = this.items.filter((item, index) => ((item.id==this.current_material_id)));
    ...
    computed: {
        filterItems: function () {
            if((this.current_material_id===undefined) || (this.current_material_id==null) || (this.current_material_id==''))
            {
                let items = this.items.filter((item, index) => (!(item.name.toLowerCase()).indexOf(this.itemsearch.toLowerCase()) || (item.id==this.itemsearch)));
